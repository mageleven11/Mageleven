...
namespace Mageleven\Core\Helper;
...
//    const DEV_ENV = ['localhost', 'dev', '127.0.0.1', '192.168.', 'demo.'];
    ...
    {
        if ($this->_magelevenModules === null) {
            $this->_magelevenModules = [];
            ...
            if ($allowList) {
                ...
                foreach ($moduleList as $name) {
                    if (strpos($name, 'Mageleven_') === false) {
                        continue;
                    }

                    $this->_magelevenModules[] = $name;
                }
            }
        }

        return $this->_magelevenModules;
    }
}

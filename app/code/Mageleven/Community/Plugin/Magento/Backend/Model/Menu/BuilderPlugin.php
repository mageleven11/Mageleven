...
namespace Mageleven\Community\Plugin\Magento\Backend\Model\Menu;

use Magento\Backend\Model\Menu\Builder;
use Magento\Backend\Model\Menu;
use Magento\Backend\Model\Menu\ItemFactory;
use Mageleven\Community\Model\Config;
...
class BuilderPlugin
{
    ...
    public function __construct(
        ...
        $this->magelevenModules = $this->getMagelevenModules();
    }
    ...
    public function afterGetResult(Builder $subject, Menu $menu, $result)
    {
        ...
        if ($menuEnabled) {
            $item = $this->menuItemFactory->create([
                'data' => [
                    'id' => 'Mageleven_Community::elements',
                    'title' => 'Mageleven',
                    'module' => 'Mageleven_Community',
                    'resource' => 'Mageleven_Community::elements'
                ]
            ]);
            $menu->add($item, null, 61);
            $subItems = $this->getSubItem($menu->toArray());
            $this->createMenuItem($menu, $subItems, 'Mageleven_Community::elements');

            $item = $this->menuItemFactory->create([
                'data' => [
                    'id' => 'Mageleven_Community::extension_and_notification',
                    'title' => 'Extensions &amp; Notifications',
                    'module' => 'Mageleven_Community',
                    'resource' => 'Mageleven_Community::elements'
                ]
            ]);
            $menu->add($item, 'Mageleven_Community::elements', 1000);

            $item = $this->menuItemFactory->create([
                'data' => [
                    'id' => 'Mageleven_Community::extension_and_notification_view',
                    'title' => 'Manage',
                    'module' => 'Mageleven_Community',
                    'resource' => 'Mageleven_Community::elements',
            ...
            $menu->add($item, 'Mageleven_Community::extension_and_notification', 1000);

            unset($this->configSections['Mageleven_Community']);

            foreach ($this->magelevenModules as $moduleName) {
                ...
                if (isset($section['id']) && 'mfextension' != $section['id']) {
                    ...
                    $menu->add($item, 'Mageleven_Community::elements');
    ...
    private function getConfigSections($moduleName)
    {
        if (null === $this->configSections) {
            ...
            foreach ($tabs as $tab) {
                if (in_array($tab->getId(), ['mageleven', 'mf_extensions_list'])) {
                    $sections = array_merge($sections, $tab->getData()['children']);
                }
            }

            foreach ($sections as $key => $section) {
                if (empty($section['resource']) || 0 !== strpos($section['resource'], 'Mageleven_')) {
                    continue;
                }
    ...
    private function createMenuItem($menu, $items, $parentId)
    {
        foreach ($items as $item) {
            ...
            $title = trim(str_replace('Mageleven_', '', $title));
            $needCreateMenuItem = ('Mageleven_Community::elements' == $parentId && !empty($item['action']));
            ...
            if ('Mageleven_Community::elements' == $parentId) {
                $addConfig = true;
                if (!empty($item['sub_menu'])) {
            ...
            $index = array_search($moduleName, $this->magelevenModules);
            if (false !== $index) {
                unset($this->magelevenModules[$index]);
            }
        }
    }
    ...
    private function getSubItem($items)
    {
        ...
        if (!empty($items)) {
            foreach ($items as $item) {
                if (isset($item['module']) &&  0 === strpos($item['module'], 'Mageleven_')
                    || !isset($item['module']) && isset($item['id']) && 0 === strpos($item['id'], 'Mageleven_')
                ) {
                    if ('Mageleven_Community::elements' != $item['id']) {
                        $subItems[] = $item;
                    }
    ...
    /**
     * Retrieve Mageleven modules info
     *
     * @return array
     */
    private function getMagelevenModules()
    {
        $modules = [];
        foreach ($this->moduleList->getNames() as $moduleName) {
            if (strpos($moduleName, 'Mageleven_') !== false && $this->moduleManager->isEnabled($moduleName)) {
                $modules[] = $moduleName;
            }
        }
        return $modules;

...
namespace Mageleven\AutoRelatedProduct\Model\ResourceModel\Rule;
...
class Collection extends AbstractCollection implements RelatedCollectionInterface
{
     ...
     * @var string
     */
    protected $_eventPrefix = 'mageleven_arp_rule_collection';
    ...
    {
        $this->_init(\Mageleven\AutoRelatedProduct\Model\Rule::class, \Mageleven\AutoRelatedProduct\Api\RelatedResourceModelInterface::class);
    ...
    protected function _renderFiltersBefore()
    {
        foreach (['store', 'group'] as $key) {
            if ($this->getFilter($key)) {
                ...
                $this->getSelect()->join(
                    [$key . '_table' => $this->getTable('mageleven_autorp_rule_' . $key)],
    ...
    protected function _afterLoad()
    {
      /*  $items = $this->getColumnValues('id');
        if (count($items)) {
            ...
            $tableName = $this->getTable('mageleven_autorp_rule_store');

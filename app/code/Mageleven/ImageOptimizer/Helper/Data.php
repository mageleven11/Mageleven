...
namespace Mageleven\ImageOptimizer\Helper;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     */
    public $indexFilename = 'mageleven_imageoptimizer_index.data';
    ...
    {
        $moduleCode = 'Mageleven_ImageOptimizer';
    ...
    public function getModuleDir()
    {
        if ($this->moduleDir === null) {
            $moduleName = 'Mageleven_ImageOptimizer';
    ...
    public function isLoggingEnabled()
    {
        if ($this->logging === null) {
            $this->logging = (int) $this->getConfig(
                'mageleven_imageoptimizer/utility/log_output'
            );
        }
    ...
    public function getPaths()
    {
        ...
        $pathsString = trim(
            trim(
                $this->getConfig('mageleven_imageoptimizer/general/paths'),
                ';'
            )
        );
    ...
    public function optimizeFile($filePath)
    {
        ...
        $permissions = (string) $this->getConfig(
            'mageleven_imageoptimizer/utility/permissions'
        );
    ...
    public function optimize()
    {
        ...
        $batchSize = (int) $this->getConfig(
            'mageleven_imageoptimizer/general/batch_size'
        );
    ...
    public function getUtil($type, $filePath)
    {
        $exactPath = $this->getConfig(
            'mageleven_imageoptimizer/utility/' . $type . '_path'
        );
        ...
        // Use path to extension's local utilities
            ...
            $cmd = $this->getUtilPath()
                . '/'
                . $this->getConfig('mageleven_imageoptimizer/utility/' . $type)
                . $this->getUtilExt();
        }
        
        $cmd .= ' ' . $this->getConfig(
            'mageleven_imageoptimizer/utility/' . $type . '_options'
        );
    ...
    public function getUtilPath()
    {
        if ($this->utilPath === null) {
            $useSixtyFourBit = (int) $this->getConfig(
                'mageleven_imageoptimizer/utility/use64bit'
            );
            ...
            $pathString = trim(
                trim($this->getConfig('mageleven_imageoptimizer/utility/path')),
                '/'
            );

 ...
 */
namespace Mageleven\AdminUserGuide\Model;
...
class XmlReader
{
    ...
    const DATA_FILE_NAME = 'mageleven/aug-data/knowledge-base.xml';
    ...
    public function get()
    {
        if (!$this->cache->load('mageleven_aug_data')) {
            $this->readFile();
        }

        try {
            return $this->json->unserialize($this->cache->load('mageleven_aug_data'));
        } catch (\Exception $e) {
            return [];
        }
    }
    ...
    private function readFile()
    {
        ...
        $this->cache->save($this->json->serialize($data),'mageleven_aug_data');
    }
